...
Depending on your needs you can <<components-extending-base-types,extend one of the base types>> or create a <<components-custom-component-type,fully custom implementation>>.
...
See <<components-custom-component-type,creating a custom component type>> if you want to have a custom metadata class, change serialization or admin UI rendering.
...
See <<components-custom-component-type,create a custom component type>> for more information.
...
The default administration UI for components supports configuring the labels.
See the xref:services-and-components/message-codes.adoc[message codes overview].

...
.Specifying (metadata) properties
...
<div id="attributes" wcm:component="attributes" wcm:type="image-with-alt" wcm:auto-create
     wcm:prop:sortIndex="10" wcm:attr:title="Attributes example"
...
In this case, the properties `sortIndex` and `title` of the component will be modified as well as the properties `altText` and `image` on the metadata instance attached to the component.

[NOTE]
====
Properties can also be defined using a nested element syntax, for example:

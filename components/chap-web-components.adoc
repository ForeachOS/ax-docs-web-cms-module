...
A `WebCmsComponentModelHierarchy` is attached to the WebApplicationContext of the current request and allows components to be looked up in the various scopes. By default the `global` and `domain` scope are registered. Using the WebCmsComponentModelHierarcyh, it is possible to register additional scopes, attach/replace/remove components to a scope and define aliases for a scope.
